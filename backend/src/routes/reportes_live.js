...
router.get('/ficha-seccion/:numero', async (req, res) => {
  ...
  const [historico, estructura, casillas, actividad30d, incidencias, riesgo, eventos, promotoresSeccion, reunionesSeccion, materialesSeccion] = await Promise.all([
    ...
    // 🆕 QUIÉN ha trabajado esta sección de verdad — no solo quien
    // está formalmente asignado ahí, sino quien REALMENTE capturó
    // promovidos aquí, y de qué coordinador/rama depende cada uno.
    query(
      `SELECT u.id, u.nombre, u.rol, u.puesto, jefe.nombre as jefe_directo, COUNT(p.id) as promovidos_aqui
       FROM promovidos p JOIN usuarios u ON u.id = p.registrado_por
       LEFT JOIN usuarios jefe ON jefe.id = u.parent_id
       WHERE p.campana_id=$1 AND p.seccion_id=$2
       GROUP BY u.id, u.nombre, u.rol, u.puesto, jefe.nombre ORDER BY promovidos_aqui DESC`,
      [campanaId, seccion.id]
    ).catch(() => ({ rows: [] })),
    // 🆕 Reuniones realizadas en esta sección — fuente: agenda
    ...
    // 🆕 Materiales/utilitarios puestos en esta sección, con su costo — fuente: activos
  ...
  // 🆕 Inversión y rentabilidad — cuánto se le ha puesto a esta
  // sección en materiales (activos.costo), y si eso se traduce en
  // resultados reales (comprometidos). Nunca inventa un ROI
  // "oficial" — es un indicador simple y honesto, costo por
  // comprometido, para ayudar a decidir, no una verdad absoluta.
  const inversionMateriales = materialesSeccion.rows.reduce((s, m) => s + (parseFloat(m.costo) || 0), 0);
  const comprometidosSeccion = parseInt(promovidosTotal.rows[0].comprometidos);
  const costoPorComprometido = comprometidosSeccion > 0 ? +(inversionMateriales / comprometidosSeccion).toFixed(2) : null;
  ...
  res.json({
    ...
    data: {
      ...
      actividad: { ultimos_30_dias: actividad30, total_promovidos: totalPromovidos, comprometidos: comprometidosSeccion },
      ...
      // 🆕 Quién ha trabajado aquí de verdad, y de qué rama depende cada uno
      promotores_por_estructura: promotoresSeccion.rows,
      // 🆕 Reuniones realizadas en esta sección
      reuniones: reunionesSeccion.rows[0],
      // 🆕 Inversión y rentabilidad
      inversion: {
        materiales: materialesSeccion.rows,
        inversion_total_materiales: inversionMateriales,
        comprometidos: comprometidosSeccion,
        costo_por_comprometido: costoPorComprometido,
        nota: 'Solo incluye el costo de materiales (activos) puestos en esta sección — no incluye gastos generales de campaña que no se puedan atribuir a un territorio específico.',
      },
    },
  });
});
...
/**
 * GET /api/reportes/pdf/bitacora-cierre
 * Reporte de cierre de jornada de la Bitacora Diaria: resumen del dia,
 * incidencias abiertas y linea de tiempo completa de eventos.
 */
router.get('/pdf/bitacora-cierre', async (req, res) => {
  const campanaId = req.usuario.campana_id;
  const fecha = req.query.fecha || new Date().toISOString().slice(0, 10);
  ...
  const eventos = await query(
    `SELECT be.*, u.nombre as usuario_nombre, s.numero as seccion_numero, m.nombre as municipio_nombre
     FROM bitacora_eventos be
     LEFT JOIN usuarios u ON u.id = be.usuario_id
     LEFT JOIN secciones s ON s.id = be.seccion_id
     LEFT JOIN municipios m ON m.id = be.municipio_id
     WHERE be.campana_id=$1 AND be.creado_en::date = $2::date
     ORDER BY be.creado_en ASC`,
    [campanaId, fecha]
  );

  const porTipo = await query(
    `SELECT tipo, COUNT(*) as total FROM bitacora_eventos
     WHERE campana_id=$1 AND creado_en::date = $2::date
     GROUP BY tipo`,
    [campanaId, fecha]
  );

  const pendientesRes = await query(
    `SELECT COUNT(*) as total FROM bitacora_eventos
     WHERE campana_id=$1 AND tipo='pendiente' AND estado != 'resuelto'`,
    [campanaId]
  );
  const pendientesAbiertos = parseInt(pendientesRes.rows[0].total);

  const incidencias = await query(
    `SELECT i.*, s.numero as seccion_numero FROM incidencias i LEFT JOIN secciones s ON s.id=i.seccion_id
     WHERE i.campana_id=$1 AND i.estado != 'resuelta' ORDER BY i.creado_en DESC`,
    [campanaId]
  );
  ...
  const doc = iniciarPDF(
    res,
    'reporte_cierre_jornada.pdf',
    'Cierre de Jornada — Bitácora Diaria',
    `${campanaRes.rows[0]?.nombre_candidato} — ${fechaFormateada}`
  );
  ...
  seccionPDF(doc, 'Resumen del día');
  lineaPDF(doc, 'Total de eventos', eventos.rows.length);
  porTipo.rows.forEach((r) => lineaPDF(doc, etiquetasTipo[r.tipo] || r.tipo, r.total));
  lineaPDF(doc, 'Pendientes abiertos (total)', pendientesAbiertos);

  seccionPDF(doc, 'Incidencias abiertas');
  if (incidencias.rows.length === 0) {
    doc.text('Sin incidencias abiertas.');
  } else {
    incidencias.rows.forEach((i) => {
      doc.font('Helvetica-Bold').text(`${i.tipo} - ${i.urgencia}${i.seccion_numero ? ` (Sección ${i.seccion_numero})` : ''}`);
      doc.font('Helvetica').fontSize(9).text(i.descripcion, { indent: 15 });
      doc.fontSize(10).moveDown(0.3);
    });
  }

  seccionPDF(doc, 'Línea de tiempo del día');
  if (eventos.rows.length === 0) {
    doc.text('Sin registros este día.');
  } else {
    eventos.rows.forEach((e) => {
      const hora = new Date(e.creado_en).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' });
      const ubicacion = e.seccion_numero
        ? ` (Sección ${e.seccion_numero}${e.municipio_nombre ? `, ${e.municipio_nombre}` : ''})`
        : e.municipio_nombre ? ` (${e.municipio_nombre})` : '';
      doc.font('Helvetica-Bold').text(`${hora} — ${etiquetasTipo[e.tipo] || e.tipo}: ${e.titulo}${ubicacion}`);
      doc.font('Helvetica').fontSize(9).text(`Registrado por: ${e.usuario_nombre || 'N/D'}`, { indent: 15 });
      if (e.descripcion) {
        doc.font('Helvetica').fontSize(9).text(e.descripcion, { indent: 15 });
      }
      doc.fontSize(10).moveDown(0.3);
    });
  }
...
router.get('/resumen-ejecutivo-ia', async (req, res) => {
  ...
  const promosRes = await query(`SELECT clasificacion, comprometido, creado_en, seccion_id FROM promovidos WHERE campana_id=$1`, [campanaId]);
  ...
  const estructuraPorRol = estructuraRes.rows.reduce((acc, r) => ({ ...acc, [r.rol]: parseInt(r.total) }), {});
  const totalEstructura = estructuraRes.rows.reduce((s, r) => s + parseInt(r.total), 0);

  const coberturaRes = await query(
    `SELECT COUNT(DISTINCT c.seccion_id) as total_secciones,
            COUNT(DISTINCT c.seccion_id) FILTER (WHERE EXISTS (SELECT 1 FROM promovidos p WHERE p.seccion_id = c.seccion_id AND p.campana_id=$1)) as secciones_con_promovidos
     FROM casillas c WHERE c.campana_id=$1`,
    [campanaId]
  );
  const cobertura = coberturaRes.rows[0];

  const coordinadoresInactivosRes = await query(
    `SELECT u.nombre, u.rol, MAX(p.creado_en) as ultima_actividad
     FROM usuarios u LEFT JOIN promovidos p ON p.registrado_por = u.id AND p.campana_id=$1
     WHERE u.campana_id=$1 AND u.activo != false AND u.rol != 'promotor'
     GROUP BY u.id, u.nombre, u.rol
     HAVING MAX(p.creado_en) IS NULL OR MAX(p.creado_en) < now() - interval '7 days'`,
    [campanaId]
  );
  const alertas = [];
  if (coordinadoresInactivosRes.rows.length > 0) {
    alertas.push(`⚠️ ${coordinadoresInactivosRes.rows.length} coordinador(es)/lider(es) sin ninguna captura en los ultimos 7 dias: ${coordinadoresInactivosRes.rows.slice(0, 5).map((c) => c.nombre).join(', ')}${coordinadoresInactivosRes.rows.length > 5 ? '...' : ''}`);
  }
  if (incidenciasActivas > 0) alertas.push(`incidencias activas sin resolver: ${incidenciasActivas}`);
  if (tope && (gastoTotal / tope) > 0.8) alertas.push(`gasto: ya usaste ${Math.round((gastoTotal / tope) * 100)}% de tu tope de gasto permitido`);
  if (cobertura.total_secciones > 0 && (cobertura.secciones_con_promovidos / cobertura.total_secciones) < 0.5) {
    alertas.push(`cobertura baja: menos de la mitad de tus secciones (${cobertura.secciones_con_promovidos}/${cobertura.total_secciones}) tienen al menos 1 promovido capturado`);
  }
  ...
  const datosParaIA = {
    ...
    estructura_por_rol: estructuraPorRol, total_estructura: totalEstructura,
    cobertura_secciones: `${cobertura.secciones_con_promovidos}/${cobertura.total_secciones}`,
  ...
  try {
    const respuesta = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Eres analista de campana politica en Mexico. Con estos datos REALES de la campana, escribe un resumen ejecutivo de 4-5 parrafos cortos, en espanol claro (no tecnico), dirigido al candidato, como si se lo explicaras en persona.

Datos:
${JSON.stringify(datosParaIA, null, 2)}

Estructura sugerida:
1. Un parrafo de "como vamos" en general (avance hacia la meta, ritmo).
2. Un parrafo sobre el equipo/estructura completa (todos los roles, no solo promotores) y cobertura territorial.
3. Un parrafo de finanzas si hay tope configurado.
4. Un parrafo sobre las alertas activas, si las hay, explica que significan, no solo las repitas.
5. Un parrafo final con 1-2 recomendaciones concretas y accionables.

Reglas OBLIGATORIAS:
- Usa UNICAMENTE los numeros que te di arriba, nunca inventes una cifra, porcentaje, o dato que no este en el JSON.
- Si algun dato viene como null, dilo abiertamente en vez de omitirlo o inventarlo.
- Se directo y honesto, no autocomplaciente, si el ritmo no alcanza para la meta, dilo con claridad.
- No repitas los numeros en formato de lista, integralos naturalmente en la narrativa.`,
      }],
    });
