...
function ModalResponderCampo({ encuesta, onCerrar }) {
  ...
  const guardar = async () => {
    await api.post(`/encuestas/${encuesta.id}/responder`, { respuestas, seccion_numero: seccionNumero ? parseInt(seccionNumero) : undefined });
    onCerrar();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-end md:items-center justify-center z-50" onClick={onCerrar}>
      <div className="bg-slate-900 border border-slate-700 rounded-t-2xl md:rounded-2xl w-full max-w-md p-5 space-y-3 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <input placeholder="Sección donde estás (opcional, para el mapa)" type="number" value={seccionNumero} onChange={(e) => setSeccionNumero(e.target.value)}
...
export default function PanelEncuestas() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-slate-400 uppercase">📋 Encuestas para tus promotores</h2>
        <button onClick={() => setMostrarCrear(true)} className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-bold">+ Nueva</button>
      </div>
